...
const emailService = {
  sendVerificationEmail: async (email, username, token) => {
    try {
      // Development modda mock email kullan
      if (process.env.NODE_ENV !== 'production') {
        console.log('📧 MOCK VERIFICATION EMAIL (Development Mode):')
      ...
      console.log(`To: ${email}`)
      console.log(`Token: ${token}`)
      console.log(`Link: http://localhost:5000/verify-email?token=${token}`)
      return { success: true, messageId: 'mock-' + Date.now() }
    }
  }
}
...
app.use(cors({
  origin: process.env.NODE_ENV === 'production' 
    ...
    : ['http://localhost:3000', 'http://localhost:5173', 'http://localhost:80', 'http://localhost:5000'],
  credentials: true
}))
...
const authenticateToken = (req, res, next) => {
  ...
  const jwtSecret = process.env.JWT_SECRET || 'serpyx-super-secret-jwt-key-2024'
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Check if email is verified (ENABLE THIS IN PRODUCTION)
    if (!user.isVerified) {
      return res.status(401).json({
        success: false,
        message: 'Lütfen önce e-posta adresinizi doğrulayın. E-posta doğrulama linkini kontrol edin.'
      })
    }
    ...
    // Generate JWT token
    const jwtSecret = process.env.JWT_SECRET || 'serpyx-super-secret-jwt-key-2024'
    const jwtRefreshSecret = process.env.JWT_REFRESH_SECRET || 'serpyx-super-secret-refresh-key-2024'
...
app.post('/api/update-stats', authenticateToken, (req, res) => {
  try {
    const { coins, highScore, totalScore } = req.body
    const userId = req.user.userId
...
app.listen(PORT, () => {
  ...
  console.log(`📧 Email service: REAL GMAIL (serpyx0@gmail.com)`)
